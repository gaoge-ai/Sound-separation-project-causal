...
def parse_dirname(dirname):
    valid_classes = set()
    if 'speech' in dirname:
        valid_classes.add('speech')
    if 'music' in dirname:
        valid_classes.add('music')
    if 'others' in dirname:
        valid_classes.add('others')
    return valid_classes


def main():
    results_dir = "./test_results/3class_nosing.causal_tmp/2mix/"
    num_samples = 5
    
    print("=" * 60)
    print("Calculate SDR/SI-SDR from Separated Results (Dirname-based)")
    print("=" * 60)
    ...
    for sample_idx, sample_dir in enumerate(tqdm(sample_dirs, desc="Processing samples")):
        ...
        valid_classes_from_name = parse_dirname(sample_name)
        ...
        valid_speech_from_energy = np.sum(speech_gt ** 2) > eps
        valid_music_from_energy = np.sum(music_gt ** 2) > eps
        valid_others_from_energy = np.sum(others_gt ** 2) > eps
        
        valid_speech = valid_speech_from_energy and ('speech' in valid_classes_from_name)
        valid_music = valid_music_from_energy and ('music' in valid_classes_from_name)
        valid_others = valid_others_from_energy and ('others' in valid_classes_from_name)
        
        print(f"\nSample {sample_idx}: {sample_name}")
        print(f"  Classes from dirname: {sorted(list(valid_classes_from_name))}")
        print(f"  Valid speech (name+energy): {valid_speech}")
        print(f"  Valid music (name+energy): {valid_music}")
        print(f"  Valid others (name+energy): {valid_others}")
    ...
    print("\n" + "="*60)
    print("Final Statistics (Valid Classes Only - From Dirname + Energy)")
    print("="*60)
